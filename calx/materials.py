...
amorphous["SiC"] = xu.materials.material.Amorphous(
    "Silicon carbide", 3210, [("Si", 1), ("C", 1)]
)
amorphous["YAG"] = xu.materials.material.Amorphous(
    "YAG", 4560, [("Y", 3), ("Al", 5), ("O", 12)]
)
amorphous["steel304"] = xu.materials.material.Amorphous(
    "steel304", 7900, [("Fe", 0.68), ("Cr", 0.2), ("Ni", 0.1), ("Mn", 0.02)]
)
amorphous["PMMA"] = xu.materials.material.Amorphous(
    "PMMA", 1190, [("C", 1), ("S", 1), ("H", 8), ("O", 2)]
)
...
def get_CS_Photo(ID, E=None):
    ...
    for id, atoms in form.atoms.items():
        CS += (
            xraylib.CS_Photo(elementZ[id.symbol], E)
            * atoms
            * AtomicMass[id.symbol]
            / c["NA"]
            / u["cm"] ** 2
        )
    return CS
